...
**runbookgen** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `runbookgen` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
runbookgen ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
runbookgen ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
runbookgen ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `runbookgen` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="runbookgen") for rec in records]   # records = your JSON output
